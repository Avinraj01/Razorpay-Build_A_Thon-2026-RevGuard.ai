...
class RevenueResilienceAgent:
    ...
    def _get_relevant_context(self, current_event: PaymentEvent) -> List[PaymentEvent]:
        # Normalize current timestamp to offset-naive for safe comparison
        curr_ts = current_event.timestamp
        if hasattr(curr_ts, "tzinfo") and curr_ts.tzinfo is not None:
            curr_ts = curr_ts.replace(tzinfo=None)

        cutoff_time = curr_ts - timedelta(minutes=self.context_window_minutes)

        filtered = []
        for e in self.recent_failures:
            e_ts = e.timestamp
            if hasattr(e_ts, "tzinfo") and e_ts.tzinfo is not None:
