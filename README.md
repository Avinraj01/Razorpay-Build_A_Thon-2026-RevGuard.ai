...
```mermaid
flowchart LR
    A[Payment Failure] --> B{Naive AI Agent?}
    B -->|Direct API Keys| C[Hallucinated Double Charges]
    B -->|Blind Retries| D[Bank Penalty Fees & Blacklisting]
    
    A --> E[RevGuard AI Engine]
    E --> F[1. Probabilistic LLM Diagnosis]
    F --> G[2. Deterministic Policy Gate]
    G --> H[3. SQLite WAL At-Most-Once Lock]
    H --> I[Verified Revenue Recovery]
```
...
mindmap
  root((RevGuard AI))
    ...
    Deterministic Safety Boundary
      Policy Engine
      Economic Floor > 100 INR
      Bounded Retries max 2
      Compliant Escalation
    Idempotency & Storage
      SQLite WAL Single-Writer
      Primary Key Unique Locks
      Immutable Reservation Ledger
      Execution History
    Adversarial Chaos Lab
      10x Concurrent Webhooks
      Crashed Worker Stale Lock
      Duplicate Dispatch PK Lock
    Batch Benchmark
      277 Held-Out Test Events
...
flowchart LR
    subgraph INGESTION["1. Ingestion Layer"]
        EV[Payment Failure Event] --> ORCH[FastAPI Orchestrator]
    end

    subgraph PROBABILISTIC["2. Probabilistic Layer (Zero Execution)"]
        ORCH -->|Raw Telemetry| LLM[LLM Diagnosis Agent]
        LLM -->|Typed Proposal + Confidence| ORCH
    end

    subgraph DETERMINISTIC["3. Deterministic Policy Boundary"]
        ORCH --> PE[Policy Engine Gate]
        PE -->|Lock Check| WAL[(SQLite WAL Store)]
        PE -->|Evaluate Invariants| PE
    end

    subgraph EXECUTION["4. Gateway Execution Layer"]
        PE -->|Approved Action| EXEC[Razorpay Executor]
        EXEC -->|PK Constraint Lock| WAL
        EXEC -->|Dispatch Action| GW[Razorpay Gateway API]
    end
```
...
sequenceDiagram
    ...
    participant WAL as SQLite WAL Store
    participant LLM as LLM Agent
    participant POL as Policy Engine
    participant EX as Razorpay Executor

    GW->>API: Ingest Failed Payment Event
    API->>WAL: Acquire PENDING lock (event_id, attempt)
    alt Lock Already Exists
        WAL-->>API: Duplicate Detected (Reject)
        API-->>GW: Return Cached Execution State
    else Lock Acquired
        WAL-->>API: Lock Granted
        API->>LLM: Diagnose Root Cause
        LLM-->>API: DiagnosisProposal(Class, Confidence)
        API->>POL: Evaluate Invariants (Amount, Retries, Policy)
        POL-->>API: PolicyDecision(Action, Reason, Gates)
        API->>EX: Dispatch Recovery Action
        EX->>WAL: Commit Permanent Execution Signature
        EX-->>API: Gateway Execution Outcome
        API-->>GW: Recovery Verified
    end
```
...
```mermaid
erDiagram
    PAYMENT_RESERVATIONS ||--o{ EXECUTION_HISTORY : executes

    PAYMENT_RESERVATIONS {
        string reservation_id PK "Key: group_id:RECOVERY:version"
        string event_id "Foreign reference to payment"
        string status "PENDING, SUCCESS, ESCALATED"
        string decision_reason "Deterministic policy output"
        int retry_count "Bounded counter (max 2)"
        datetime created_at "Immutable UTC timestamp"
    }

    EXECUTION_HISTORY {
        string execution_id PK "UUID primary key"
        string reservation_id FK "References reservation"
        string action_type "RETRY, OFFER_ALTERNATE_METHOD"
        string outcome "SUCCESS, LINK_SENT, BLOCKED"
        string razorpay_payment_id "Gateway reference signature"
        int latency_ms "Execution latency"
        datetime executed_at "Committed UTC timestamp"
    }
```
